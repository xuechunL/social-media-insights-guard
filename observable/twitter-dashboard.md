...
```js
const versions = FileAttachment('data/plot-version-data.csv').csv({
	typed: true,
})
const downloads = FileAttachment('data/plot-npm-downloads.csv').csv({
	typed: true,
})
const issues = FileAttachment('data/plot-github-issues.json')
	.json()
	.then(revive)
...
# X (fka Twitter) GitHub Community <img src="./github-mark.svg" width="32" height="32" />

[X (fka Twitter)](https://github.com/twitter) is what's happening and what people are talking about right now. It's where news breaks and communities converge. And it's all powered by open source, made by a team of amazing humans.
